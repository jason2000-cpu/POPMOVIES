...
Class Actions extends DBConnection{
    ...
    function save_application(){
        ...
        $_POST['username'] = ucwords($_POST['lastname']);
        if(empty($_POST['id'])){
            $code = time();
            while(true){
        ...
        $data = "";
        foreach($_POST as $k => $v){
            if(in_array($k,array('plan_id','application_code','username','status'))){
                if(!is_numeric($v))
                $v = $this->escapeString($v);
                if(empty($id)){
                    $cols[] = $k;
        ...
        if($save){
            ...
            $data = "";
            foreach($_POST as $k => $v){
                if(!in_array($k,array('id','plan_id','application_code','username','status'))){
                    if(!is_numeric($v))
                    $v = $this->escapeString($v);
                    if(!empty($data)) $data .=", ";
                    $data .="('{$id}', '{$k}','{$v}')";
